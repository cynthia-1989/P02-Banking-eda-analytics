...
#   Which specific banking transactions have amounts that are statistically unusual?
#   Which customer accounts have balances or credit values that no normal account should produce?
#
# THE BUSINESS QUESTION:
#   "We know the average transaction amount and account balance. But are there any
#    transactions or customer accounts that are so far from normal that we should
#    investigate them? Either they are legitimate high-value banking activities,
#    possible fraud/risk cases, or the record is wrong."
#
# THE ANALOGY:
#   Imagine plotting all transaction amounts on a number line.
...
class AnomalyDetector:
    ...
    def _detect_zscore(self, col: str) -> pd.Series:
        ...
        LIMITATION:
        ───────────
        If the data is highly skewed, the mean and std are themselves pulled by
        extreme values, making Z-scores unreliable. Banking transaction data can
        be skewed because a few high-value transactions may be much larger than
        normal customer activity. This is why we combine Z-score with IQR.
    ...
    def run(self, columns: list = None) -> "AnomalyDetector":
        ...
        if columns:
            ...
            target_cols = [col for col in banking_default_cols if col in num_cols]
        ...
        for col in target_cols:
            ...
            if col not in num_cols:
                logger.warning(f"[ANOMALY] Column '{col}' is not numeric — skipping")
                continue
    ...
    def save_anomalies(self) -> "AnomalyDetector":
        ...
        Anomaly rows need to be reviewed by a human:
          ...
          - Is this a legitimate high-value transaction? → keep, flag for modelling
          - Is this a possible banking risk/fraud case? → send for business review
    ...
    def summary(self) -> pd.DataFrame:
        ...
        for col, stats in self.results.items():
            rows.append({
                "column":               col,
                "iqr_flagged":          stats["iqr_flagged"],
                "zscore_flagged":       stats["zscore_flagged"],
