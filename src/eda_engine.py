...
#   The Banking Analytics team wants to know:
#     - How are transaction amounts distributed across account types?
#     - Do account balances and transaction amounts correlate as expected?
#     - Are there time trends in banking transactions or customer activity?
#
# THE ANALOGY:
#   Imagine you just received a report from every bank branch.
#   Before presenting to senior management, you need to read it, find the patterns,
...
from config import (
    ...
    TOP_N_GROUPS,          # how many top groups to show
    CORRELATION_THRESHOLD, # minimum r to include
...
class EDAEngine:
    ...
    Five methods, each answering a different business question:
      ...
      3. group_analysis() → How do banking metrics vary by account or customer category?
    ...
    def load(self) -> "EDAEngine":
        ...
        if not DATA_PATH.exists():
            raise FileNotFoundError(
                ...
                "Run Module 05 first:\n"
                "  python run.py\n"
                "Then confirm processed-data.csv exists in data/processed/"
            )
    ...
    def profile(self) -> "EDAEngine":
        ...
        WHY PROFILE FIRST?
        ────────────────────
        Before asking "which account type has the highest transaction amount?"
        you need to know:
        "Do we have transaction data for all customers, or is 30% missing?"
        ...
        Example: transaction amounts [50, 60, 70, 80, 25000]
          mean   = 5,052 — distorted by the extreme
          median = 70    — the genuine middle transaction amount
    ...
    def group_analysis(self) -> "EDAEngine":
        ...
        Every business stakeholder cares about group differences:
          Banking: transaction performance across account types
          Finance: customer balances by branch
          Risk:    credit behaviour across customer segments
          Ops:     branch activity and operational efficiency

        HOW pd.DataFrame.groupby() WORKS:
        ───────────────────────────────────
        df.groupby("account_type")["transaction_amount"].agg(["mean", "median", "std"])
          → splits the DataFrame into one group per unique account type
          → takes the transaction amount column from each group
          → computes mean, median, std for each group
          → returns a new DataFrame with one row per account type
    ...
    def correlation(self) -> "EDAEngine":
        ...
        r ranges from -1 to +1:
          +1.0 → perfect positive relationship
                 (when account balance goes up, transaction activity also increases)
    ...
    def time_trends(self) -> "EDAEngine":
        ...
        "Average transaction amount is $9,200" — is that growing or shrinking?
        ...
        Applied to the monthly mean column:
          January mean transaction amount  = 9,200
          February mean transaction amount = 9,850
          February diff = 9,850 - 9,200 = +650
    ...
    def report(self, save: bool = True) -> None:
        ...
        This is the final step — it turns banking metrics into language
        management can read.
        A good EDA report:
        ...
        lines += [
            "═" * 65,
            f"  MODULE 06 — BANKING EDA REPORT  |  INDUSTRY: {INDUSTRY.upper()}",
            "═" * 65,
        ]
        ...
        if "correlation" in self.results:
            ...
            if pairs:
                lines += [
                    "",
                    f"  TOP BANKING CORRELATIONS (|r| > {CORRELATION_THRESHOLD})",
        ...
        if "time_trends" in self.results:
            lines += ["", "  BANKING TIME TRENDS (last 6 periods)"]
        ...
        lines += [
            "",
            "  NEXT STEPS:",
            "    → Use banking correlations for ML feature selection (Module 09)",
            "    → Pass banking profile stats to Claude for AI insights (Module 11)",
            "    → Use banking profile as drift detection baseline (Module 14)",
            "",
            "═" * 65,
        ]
