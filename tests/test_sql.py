...
def test_extract_sql_contains_industry_placeholder():
    ...
    assert "{industry}" in content, \
        "Extraction SQL must use {industry} placeholder for multi-schema support"
...
def test_query_runner_handles_bad_sql_gracefully():
    ...
    assert isinstance(df, pd.DataFrame), \
        "run() should return empty DataFrame on SQL error, not raise exception"
...
def test_extractor_synthetic_data_has_required_columns():
    ...
    for col in required:
        assert col in raw.columns, f"Synthetic banking data missing column: {col}."
    print("  PASS: test_extractor_synthetic_data_has_required_columns")


def test_extractor_synthetic_data_has_quality_issues():
    """Synthetic banking data must contain intentional data quality issues."""
    raw = DataExtractor._synthetic_raw_data(300)

    # Should have some NULL transaction amounts
    null_transactions = raw["transaction_amount"].isna().sum()
    assert null_transactions > 0, \
        "Synthetic raw banking data should have some NULL transaction_amount values"

    # Should have some invalid credit score entries
    weird_credit_scores = ((raw["credit_score"] < 300) | (raw["credit_score"] > 900)).sum()
    assert weird_credit_scores > 0, \
        "Synthetic raw banking data should have some invalid credit_score entries"
...
if __name__ == "__main__":
    print()
    print("=" * 60)
    print("  MODULE 03 — BANKING SQL UNIT TESTS")
    print("=" * 60)
    print()
    ...
    print()
    print("=" * 60)
    print("  All banking SQL tests passed ✓")
    print("=" * 60)