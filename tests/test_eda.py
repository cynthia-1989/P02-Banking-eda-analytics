...
def make_sample_df(rows: int = 50) -> pd.DataFrame:
    ...
    account_types = ["Savings", "Checking", "Business", "Student", "Credit"]
    branches = ["Calgary", "Edmonton", "Red Deer", "Airdrie", "Lethbridge"]
    
    
    transaction_count = np.random.randint(1, 20, rows)
    account_balance = np.random.uniform(100, 50000, rows).round(2)
    transaction_amount = np.random.uniform(10, 10000, rows).round(2)
    service_fee = np.random.uniform(0, 50, rows).round(2)
    interest_rate = np.random.choice([1.5, 2.0, 2.5, 3.0, 3.5], rows)

    return pd.DataFrame({
        "customer_id": range(1, rows + 1),
        "account_type": [account_types[i % len(account_types)] for i in range(rows)],
        "branch_name": [branches[i % len(branches)] for i in range(rows)],
        "transaction_count": transaction_count,
        "account_balance": account_balance,
        "transaction_amount": transaction_amount,
        "service_fee": service_fee,
        "interest_rate": interest_rate,
        "payment_channel": np.random.choice(["ATM", "Online", "Branch", "Mobile"], rows),
        "customer_segment": np.random.choice(["Retail", "Premium", "Business"], rows),
    })
...
def test_anomaly_detector_flags_obvious_outlier():
    ...
    df.loc[0, "transaction_amount"] = 10_000_000

    detector = AnomalyDetector(df)
    detector.run(columns=["transaction_amount"])
...
def test_anomaly_detector_clean_data_has_few_anomalies():
    ...
    np.random.seed(42)
    df = pd.DataFrame({
        "transaction_amount": np.random.normal(1000, 100, 200)
    })
...
def test_anomaly_detector_original_data_unchanged():
    ...
    df = make_sample_df()
    original_amount = df.loc[0, "transaction_amount"]

    detector = AnomalyDetector(df)
    detector.run()

    assert (
    df.loc[0, "transaction_amount"] == original_amount
), "AnomalyDetector should not modify the original DataFrame"

def test_detector_handles_zero_std_column():
    """Detector should not crash when a column has all identical values (std=0)."""
    df = pd.DataFrame({
        "constant": [100.0] * 20,
        "transaction_amount": np.random.uniform(50, 500, 20),
    })

    # Should not raise ZeroDivisionError or any other exception
    try:
        detector = AnomalyDetector(df)
        detector.run(columns=["constant", "transaction_amount"])
...
if __name__ == "__main__":
    print()
    print("=" * 60)
    print("  MODULE 06 — BANKING EDA AND ANOMALY DETECTION TESTS")
    print("=" * 60)
    print()
